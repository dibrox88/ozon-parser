...
- **Структура**: каждые 3 столбца = [Цена, Название, Тип]
...
2. **Поиск похожих** - система ищет похожие товары в каталоге:
   - Точное совпадение (100% score)
   - Частичное совпадение по названию (80% score)
   - Совпадение по ключевым словам (30-70% score)
3. **Интерактивное подтверждение** (опционально) - отправка в Telegram для проверки
4. **Сохранение** - связка сохраняется в `product_mappings.json`
