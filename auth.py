...
class OzonAuth:
    ...
    def enter_sms_code(self) -> bool:
        ...
        try:
            ...
            logger.info(f"✅ Получен SMS код от пользователя: {sms_code}")
            sync_send_message(f"✅ Получено: <code>{sms_code}</code>\n\n🔄 Вводим код в форму...")
            
            # ВАЖНО: Даём больше времени на очистку всех updates
            logger.info("⏸️ Пауза 2 секунды для стабилизации...")
            time.sleep(2)
